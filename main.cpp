...
int main()
{
    ...
    while (!q.priority.empty())
    {
        ...
        if (curr_num > 1000000)
        {
            ...
            uint32_t state[4][4];
            for (int pw1=0;pw1+3<q.guesses.size();pw1+=4)
            {
                // TODO：对于SIMD实验，将这里替换成你的SIMD MD5函数
                vector<string>pw={q.guesses[pw1],q.guesses[pw1+1],q.guesses[pw1+2],q.guesses[pw1+3]};
                MD5Hash_neon(pw, state); // Replace with the existing MD5Hash function
